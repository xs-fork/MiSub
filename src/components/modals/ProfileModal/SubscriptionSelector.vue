...
<template>
  <div v-if="subscriptions.length > 0" class="space-y-2">
    <div class="flex justify-between items-center mb-2">
      <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('profileModal.selectSubscriptions') }}</h4>
      <div class="space-x-2">
    ...
    <div class="relative mb-2">
      <input type="text" v-model="searchModel" :placeholder="t('subscriptions.searchPlaceholder')"
    ...
    <div
      ...
      <div v-if="filteredSubscriptions.length === 0" class="text-center text-gray-500 text-sm py-4">
        {{ t('profileModal.noMatchedSubscriptions') }}
      </div>
    </div>
    ...
    <div v-if="orderedSelectedSubs.length > 0" class="mt-3">
      <div class="flex justify-between items-center mb-1.5">
        <h5 class="text-xs font-medium text-gray-500 dark:text-gray-400">
          {{ t('profileModal.selectedDrag', { count: orderedSelectedSubs.length }) }}
        </h5>
      </div>
  ...
  <div v-else
    ...
    {{ t('profileModal.noAvailableSubscriptions') }}
  </div>
</template>
